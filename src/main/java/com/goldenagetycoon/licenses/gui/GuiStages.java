...
import com.goldenagetycoon.licenses.config.StageConfigValues;
...
public class GuiStages extends GuiScreen {
	...
	public void initGui() {
		...
		stages = new ArrayList<>(this.info.getStages());
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (stages.size() > 0) {
			...
			for (int i = indexdisplayed; i < stages.size(); i++) {

				// Stage Layout
				if (i < 5 + indexdisplayed) {
					mc.renderEngine.bindTexture(Reference.SKILLS_RES);
					drawTexturedModalRect(left + 10, top + 16 + stageLayoutOffset, 0, 166, 157, 26);
	...
	public void handleMouseInput() throws IOException {
		...
		if (Mouse.getEventDWheel() > 0) {
			if (stages.size() > 5 && indexdisplayed > 0) {
				indexdisplayed -= 1;
			}
