package com.goldenagetycoon.licenses.gui.button;
...
public class GuiButtonInventoryTab2 extends GuiButton {
	...
	public void drawButton(Minecraft mc, int mouseX, int mouseY, float f) {
		...
		if (curr instanceof GuiContainerCreative
				&& ((GuiContainerCreative) curr).getSelectedTabIndex() != CreativeTabs.INVENTORY.getIndex()) {
			enabled = false;
		}
		...
		if (enabled) {
            ...
            if (mouseX > this.x && mouseY > this.y && mouseX < this.x + width && mouseY < this.y + height) {
                InventoryTabHandler.tooltip = new TextComponentTranslation("licenses.tab." + type.name().toLowerCase()).getUnformattedComponentText();
